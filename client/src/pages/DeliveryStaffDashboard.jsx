					...
					<div className="space-y-6">
						...
						<div className="bg-violet-900 rounded-2xl p-6 text-white shadow-lg overflow-hidden relative">
							<div className="relative z-10">
								...
								<p className="text-violet-300 text-sm">Always check the order contents before leaving the kitchen to ensure complete customer satisfaction.</p>
							</div>
...
const StatCard = ({ icon, label, value, subtext, color = "text-violet-700", bgColor = "bg-violet-50", onClick, interactive }) => (
	<div 
		onClick={onClick}
		className={`bg-white rounded-2xl border border-stone-200 p-5 shadow-sm transition-all ${interactive ? 'cursor-pointer hover:border-violet-300 hover:shadow-md active:scale-95' : ''}`}
	>
...
const ActiveDeliveryCard = ({ delivery, onUpdate }) => {
	...
	const statusConfig = {
		...
		delivered: { color: "bg-violet-50 text-violet-700 border-violet-100", label: "Delivered" },
	};
	...
	<div className="bg-violet-50 border border-violet-100 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-center gap-4 transition hover:bg-violet-100/50">
		<div className="flex-1 w-full sm:w-auto">
			<div className="flex items-center gap-2 mb-1">
				<span className="bg-violet-600 text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase">New Offer</span>
				<span className="text-stone-800 font-bold">Order #{String(offer.order?._id).slice(-6).toUpperCase()}</span>
			</div>
			<p className="text-violet-900 font-medium mb-1">{offer.order?.restaurantId?.name}</p>
