...
export default function DeliveryStaffRegistration() {
	...
	const validateForm = () => {
		...
		Object.keys(formData).forEach(key => {
			if (key !== 'role' && key !== 'staffRoad' && key !== 'referralCode') {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const payload = {
				...
				role: 'deliveryStaff',
				address: {
					house: formData.staffHouse.trim(),
					road: formData.staffRoad.trim(),
					area: formData.staffArea.trim(),
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="space-y-5 rounded-xl border-2 border-violet-100 bg-gradient-to-br from-violet-50/50 to-purple-50/30 p-6">
							...
							<div className="grid md:grid-cols-2 gap-5">
								{/* House Number */}
								<div>
									<label htmlFor="staffHouse" className="block text-sm font-medium text-gray-700 mb-2">
										House / Flat Number
									</label>
									<input
										type="text"
								...
								{/* Road / Street */}
								<div>
									<label htmlFor="staffRoad" className="block text-sm font-medium text-gray-700 mb-2">
										Road / Street
									</label>
									<input
										type="text"
							...
							<div className="grid md:grid-cols-2 gap-5">
								{/* Area */}
								<div>
									<label htmlFor="staffArea" className="block text-sm font-medium text-gray-700 mb-2">
									...
									<input
										...
										id="staffArea"
										name="staffArea"
										value={formData.staffArea}
										onChange={handleChange}
										onBlur={handleBlur}
										required
										className={`w-full rounded-lg border-2 px-4 py-3 transition-all focus:outline-none bg-white ${
											errors.staffArea ? 'border-red-300 focus:border-red-500' : 'border-gray-200 focus:border-violet-500'
										}`}
										placeholder="Gulshan, Dhanmondi"
										aria-invalid={!!errors.staffArea}
									/>
									{errors.staffArea && (
										<p className="mt-1.5 text-sm text-red-600">{errors.staffArea}</p>
									)}
								</div>
